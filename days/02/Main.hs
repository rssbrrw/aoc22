...
roundScore (oppThrow, myThrow) = myScore + resultScore
    where
        ...
        resultScore = case roundResult (oppThrow, myThrow) of
            Win  -> 6
            Lose -> 0
            Draw -> 3

requiredThrow :: (Throw, Result) -> Throw
requiredThrow (oppThrow, result) = case result of
    Draw -> oppThrow
    Lose -> case oppThrow of
        Rock     -> Scissors
        Paper    -> Rock
        Scissors -> Paper
    Win -> case oppThrow of
        Rock     -> Paper
        Paper    -> Scissors
        Scissors -> Rock
...
throwScore throw = case throw of
    Rock     -> 1
    Paper    -> 2
    Scissors -> 3
